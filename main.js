...
const interceptQueryId = page => new Promise(async(resolve, reject) => {
    ...
    setTimeout(() => {if(!resolved){reject();}}, 100000);
});
...
const queryRegionHomes = async (queryState, type) => {
    if(type === 'rent'){
        ...
        queryState.filterState = {"isRecentlySold":{"value":true},"isComingSoon":{"value":false},"isAuction":{"value":false},"isForRent":{"value":false}};
    }
    else if(type === 'all'){
        ...
        handlePageFunction: async ({ page, request, puppeteerPool }) => {
            ...
            // Split map and enqueue sub-rectangles
                ...
                for(const state of states){
					console.log('Splitting homes... State is: ' + state + ' and startUrl for this state is: ' + startUrl + '...');
                    await requestQueue.addRequest({
